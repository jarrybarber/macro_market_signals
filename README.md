...
Here I set out to see whether publicly available macroeconomic and market data could be used to predict changes in GDP and QQQ. I also attempted to create a buy/sell strategy using QQQ predictions that could beat a simple buy-and-hold strategy. The results mostly confirm the efficient market hypothesis, with the models modestly beating a "predict the training mean" baseline for near-term GDP and QQQ, but struggled to beat baseline for longer-horizon GDP (12 months) and offered only weak directional signal for QQQ overall. The resulting buy/sell strategy edged out buy-and-hold ($3.99 vs $3.68 on $1 invested over the ~7 year test period), but the margin is small enough, and the underlying model's individual predictive power weak enough, that this difference is likely simply due to chance.

My most useful takeaways were derived more from the process than the specific outputs. It was very useful practicing time-aware feature engineering (respecting real-world reporting lags to avoid lookahead bias) and walk-forward cross-validation for evaluating time series models.

Natural next steps: increasing the sampling frequency (e.g. weekly instead of monthly) for a more statistically meaningful test of the buy/sell strategy, exploring whether GDP predictions could improve QQQ predictions, and implementing PCA exploration to see if a reduced set of macro factors captures the same signal as the full feature set.